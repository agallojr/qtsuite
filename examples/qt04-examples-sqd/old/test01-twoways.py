...
from pyscf import ao2mo, tools, scf, gto, mcscf
...
def plot_sqd_results(
    energy_hist_arr: np.ndarray,
    occupancy_hist_list: list,
    iterations_for_plot: int,
    exact_energy_ref: float,
    output_path: str,
    show_plot: bool = True
    ) -> str:
    """
    Plot SQD energy error convergence and average occupancy, save to file, [and open viewer]
    ...
    x1 = range(iterations_for_plot)
    min_e = [np.min(e) for e in energy_hist_arr]
    e_diff = [abs(e - exact_energy_ref) for e in min_e]
    yt1 = [1.0, 1e-1, 1e-2, 1e-3, 1e-4]
    chem_accuracy = 0.001  # Chemical accuracy (+/- 1 milli-Hartree)

    # Data for avg spatial orbital occupancy
    y2 = occupancy_hist_list[-1][0] + occupancy_hist_list[-1][1]
    x2 = range(len(y2))
    ...
    print(f"Exact energy: {exact_energy_ref:.5f} Ha")
...
if __name__ == '__main__':
    ...
    for caseId, caseArgs in cases_to_run:
        ...
        if 'input_fcidump' in caseArgs and caseArgs['input_fcidump']:
            ...
            if 'dump_integrals' in caseArgs and caseArgs.get('dump_integrals', False):
                dump_file = globalArgs["savedir"] + "fcidump.txt"
            else:
                dump_file = None
            mol = gto.M(atom=caseArgs['atom'], basis=caseArgs['basis'],
                symmetry=caseArgs['symmetry'], spin=caseArgs['spin'], charge=caseArgs['charge'])
            molecule_scf = scf.RHF(mol).run()
            # Define active space
            n_frozen = caseArgs['freeze_active']
            active_space = range(n_frozen, mol.nao_nr())
            # Get molecular integrals
            n_frozen = caseArgs['freeze_active']
            active_space = range(n_frozen, molecule_scf.mol.nao_nr())
            scf_obj = scf.RHF(molecule_scf.mol).run()
            num_orbitals = len(active_space)
            n_electrons = int(sum(scf_obj.mo_occ[active_space]))
            num_elec_a = caseArgs['num_alpha'] #(n_electrons + molecule_scf.mol.spin) // 2
            num_elec_b = caseArgs['num_beta']  #(n_electrons - molecule_scf.mol.spin) // 2
            cas = mcscf.CASCI(scf_obj, num_orbitals, (num_elec_a, num_elec_b))
            mo = cas.sort_mo(active_space, base=0)
            core_hamiltonian, nuclear_repulsion_energy = cas.get_h1cas(mo)
            electron_repulsion_integrals = ao2mo.restore(1, cas.get_h2cas(mo), num_orbitals)
            caseArgs['num_orbitals'] = len(active_space)
            if dump_file:
                fcidump.from_scf(molecule_scf, dump_file)
                print(f"FCIDUMP: {dump_file}")
            print("Molecule created from pyscf.")
        ...
        output_path = plot_sqd_results(
            energy_hist_arr=energy_hist,        # Energies per iteration and batch (Hartree)
            occupancy_hist_list=occupancy_hist, # Average (alpha, beta) occupancies per iteration
            iterations_for_plot=caseArgs['iterations_count'], # for x-axis
            exact_energy_ref=caseArgs['exact_energy_ref'],    # reference exact energy
            output_path=output_path,            # where to save the generated figure
            show_plot=caseArgs['show_plot']     # if True, open image viewer after saving
        )
